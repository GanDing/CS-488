...
int gr_CSG_cmd(lua_State* L)
{
  ...
  gr_node_ud* geo1 = (gr_node_ud*)luaL_checkudata(L, 2, "gr.node");
  luaL_argcheck(L, geo1 != 0,2, "Node expected");
  GeometryNode* g1 = (GeometryNode*)geo1->node;

  gr_node_ud* geo2 = (gr_node_ud*)luaL_checkudata(L, 3, "gr.node");
  luaL_argcheck(L, geo2 != 0,3, "Node expected");
  GeometryNode* g2 = (GeometryNode*)geo2->node;

  int operation = luaL_checknumber(L, 4);
  data->node = new CSGNode(name, g1, g2, operation);
...
int gr_material_cmd(lua_State* L)
{
  ...
  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     ...
                                     glm::vec3(coefficient[0], coefficient[1], coefficient[2]),
                                     shininess,
                                     reflection,
                                     refraction);
...
int gr_texture_cmd(lua_State* L)
{
  ...
  double reflection = luaL_checknumber(L, 5);
  double refraction = luaL_checknumber(L, 6);
  //double fresnelk = luaL_checknumber(L, 7);
  //double np = luaL_checknumber(L, 8);


  // data->material = new TextureMaterial(glm::vec3(coeff[0],coeff[1],coeff[2]),
  //                                    picname,
  //                                    glm::vec3(ks[0], ks[1], ks[2]),
  //                                    glm::vec3(kr[0], kr[1], kr[2]),
  //                                    shininess,n,fresnelk,np);
...
int gr_bump_cmd(lua_State* L)
{
  ...
  data->bump = new BumpMaterial(fileName);
...
// Add a movement to a node.
extern "C"
int gr_node_add_move_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double isMove = luaL_checknumber(L, 2);;
  glm::vec3 posDirection;
  get_tuple(L, 3, &posDirection[0], 3);
  double posSpeed = luaL_checknumber(L, 4);

  ((GeometryNode *)self)->m_primitive->motionInfo = MotionBlur(isMove, glm::vec4(posDirection, 0.0f), posSpeed);
...
int gr_material_set_bump_cmd(lua_State* L)
{
  ...
  BumpMaterial* bumpMaterial = matdata->bump;

  self->setBumpMaterial(bumpMaterial);
